    ...
    for line in infile.readlines():
        ...
        if s_pos != -1 :
            ...
            if e_pos != -1 :
                ...
                if id_str == '' :
                    print('xtargets are errors', line, 'fn', fn, file=sys.stderr)
                    continue

                left_right = id_str.split(';') # 0, 1
                if len(left_right) != 2 : input('You should never see this: %s' %line)
                if fn == 'AL/4RT_01AL.txt' or fn == 'AL/4RT_02AL.txt' or fn == 'AL/4RT_03AL.txt' : # left id is korean id
                    if left_right[0].strip() != '':
                        ...
                        print('left_ids are empty', line, 'fn', fn, file=sys.stderr)
                    ...
                    if left_right[1].strip() != '':
                        ...
                        print('right_ids are empty', line, 'fn', fn, file=sys.stderr)
...
def proc_all(infn):
    ...
    for each in sorted(err_fn) :
        print(each, err_fn[each], file=sys.stderr)
...
if __name__ == "__main__" :
    try :
        ...
        if os.path.exists('conc') != True:
            ...
            print('Voila, under conc directory')
            os.system("mv para/*.conc conc/")
            os.system("rm para/*.id")
    except Exception as e:
        print(e)
